...
export default function CdmSimulatorActivity({ id, data, complete, onBack }) {
  const isDev = id?.startsWith('dev_');
  const targetScenarios = isDev ? DEV_ROLEPLAY_SCENARIOS : ROLEPLAY_SCENARIOS;

  const [phase, setPhase] = useState(0);
  const [intuition, setIntuition] = useState(data?.intuition ?? { risk: 3, feeling: '', action: '' });
  const [timeline, setTimeline] = useState(data?.timeline ?? ['', '', '', '']);
  const [probeAns, setProbeAns] = useState(data?.probeAns ?? {});
  const [aarAns, setAarAns] = useState(data?.aarAns ?? {});
  const [card, setCard] = useState(data?.card ?? { cue: '', interpret: '', action: '', boundary: '', alt: '' });

  const scenario = targetScenarios[2];
  const stimulus = scenario.steps[0].message;

  const handleNext = () => setPhase((prev) => Math.min(prev + 1, 5));

  const handleAutoFill = () => {
    if (isDev) {
      setCard({
        cue: '수정된 코드가 다른 스킬과 충돌하며 프롬프트 한계치에 도달',
        interpret: '공통 프롬프트에 계속 밀어 넣기보다 RAG 또는 분할 아키텍처로 넘어가야 할 시점',
        action: '신규 기능을 워크플로우 문서로 분리하고 메인 프롬프트에는 참조 지시만 남긴다',
        boundary: '너무 많은 파일 컨텍스트가 주입된 상태에서 억지로 단일 프롬프트를 확장할 때',
        alt: '기능 범위를 축소한 MVP로 다시 안정화한다',
      });
      return;
    }
    ...
    setCard({
      ...
      interpret: '단순 지각이 아니라 무단 결근 가능성을 사전에 감지',
      action: '대체 강사 1순위를 즉시 투입하고 학부모 안심 문자 초안을 준비',
      boundary: '대체 강사가 해당 반 진도와 학생 상황을 전혀 파악하지 못한 경우',
      alt: '원장이 직접 투입하고 환불 규정까지 동시에 검토',
    });
  };
          ...
          <div className="split-left">
            ...
            <div className="card" style={{ background: 'var(--primary-light)', borderColor: 'var(--primary)', marginBottom: 'var(--space-xl)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                ...
                <span style={{ fontWeight: 700, color: 'var(--primary-hover)', fontFamily: 'var(--font-mono)' }}>{scenario.title}</span>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {phases.map((phaseName, index) => (
                <div key={phaseName} className={`choice-btn ${phase >= index ? 'selected' : ''}`} style={{ padding: '16px', cursor: 'default' }}>
            ...
            <AnimatePresence mode="wait">
              {phase === 0 && (
                <motion.div key="phase0" variants={variants} initial="hidden" animate="visible" exit="exit" className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <p style={{ marginBottom: 24, fontSize: '0.875rem' }}>분석하지 말고 30초 안에 떠오르는 직관적 반응을 기록하세요.</p>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1 }}>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(event) => setIntuition((prev) => ({ ...prev, feeling: event.target.value }))}
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(event) => setIntuition((prev) => ({ ...prev, action: event.target.value }))}
                        placeholder="이유보다 몸이 먼저 반응하는 행동"
                        style={{ width: '100%', padding: '12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', resize: 'none', height: '80px' }}
                      />
                    </div>
                  </div>
              ...
              {phase === 1 && (
                <motion.div key="phase1" variants={variants} initial="hidden" animate="visible" exit="exit" className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <p style={{ marginBottom: 16, fontSize: '0.875rem' }}>의사결정 순간을 4단계 프레임으로 쪼개 복기합니다.</p>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12, flex: 1, overflowY: 'auto' }}>
                    {['첫 신호 인지', '상황 전개 및 정보 수집', '최종 분기점 (의사결정)', '결과 및 수습'].map((label, index) => (
                      <div key={label}>
                        <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--layer-c)', marginBottom: 4 }}>{index + 1}. {label}</div>
                        <input
                          value={timeline[index]}
                          onChange={(event) => {
                            const nextTimeline = [...timeline];
                            nextTimeline[index] = event.target.value;
                  ...
                  <button className="btn btn-primary" style={{ width: '100%', marginTop: 24, background: 'var(--layer-c)' }} onClick={handleNext} disabled={timeline.filter((value) => value.trim()).length < 2}>
                    다음 단계 →
                  </button>
                </motion.div>
              )}
              ...
              {phase === 2 && (
                <motion.div key="phase2" variants={variants} initial="hidden" animate="visible" exit="exit" className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, overflowY: 'auto' }}>
                    {CDM_PROBES.map((probe, index) => (
                      <div key={probe}>
                        <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--layer-b)', marginBottom: 6 }}>Q{index + 1}. {probe}</div>
                        <textarea
                          value={probeAns[index] || ''}
                          onChange={(event) => setProbeAns({ ...probeAns, [index]: event.target.value })}
                          rows={2}
              ...
              {phase === 3 && (
                <motion.div key="phase3" variants={variants} initial="hidden" animate="visible" exit="exit" className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <p style={{ marginBottom: 16, fontSize: '0.875rem' }}>After Action Review로 원래 의도와 실제 결과를 대조합니다.</p>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, overflowY: 'auto' }}>
                    {AAR_QUESTIONS.map((question, index) => (
                      <div key={question.label}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                          <span className="tag" style={{ background: 'var(--success)', color: 'white' }}>{question.label}</span>
                          <span style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-main)' }}>{question.q}</span>
                        </div>
                        <textarea
                          value={aarAns[index] || ''}
                          onChange={(event) => setAarAns({ ...aarAns, [index]: event.target.value })}
                          rows={2}
                          style={{ width: '100%', padding: '12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', fontSize: '0.875rem', resize: 'none' }}
                        />
                      </div>
              ...
              {phase === 4 && (
                <motion.div key="phase4" variants={variants} initial="hidden" animate="visible" exit="exit" className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <p style={{ marginBottom: 16, fontSize: '0.875rem' }}>지금까지 도출한 암묵지를 신입도 재사용 가능한 1장의 카드로 압축합니다.</p>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12, flex: 1, overflowY: 'auto' }}>
                    {[
                      { key: 'cue', label: '결정적 단서', placeholder: '무엇을 보고 들었는가?' },
                      { key: 'interpret', label: '해석 규칙', placeholder: '단서를 어떤 가설로 연결했는가?' },
                      { key: 'action', label: '행동 처방', placeholder: '즉시 어떤 조치를 취하는가?' },
                      ...
                      <div key={field.key} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                        ...
                        <input
                          ...
                          onChange={(event) => setCard({ ...card, [field.key]: event.target.value })}
                  ...
                  <div style={{ marginTop: 'auto' }}>
                    <ActivityFooter
                      onComplete={(insight = '') => complete({
                        activityData: { intuition, timeline, probeAns, aarAns, card, insight },
                        bonusXp: 40,
                      })}
                      onSkip={() => complete({
                        activityData: { intuition, timeline, probeAns, aarAns, card, insight: 'Skipped' },
                      })}
